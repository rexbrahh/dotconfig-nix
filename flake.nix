{
  ...
  in {
    ...
    # Convenience apps: switch, check, fmt
    apps = genAttrs supportedSystems (
      ...
      in
        ...
        // {
          check = {
            type = "app";
            program = "${pkgs.writeShellScriptBin "flake-check" ''
              set -euo pipefail
              nix flake check
            ''}/bin/flake-check";
          ...
          fmt = {
            type = "app";
            program = "${pkgs.writeShellScriptBin "flake-fmt" ''
              set -euo pipefail
              nix fmt
            ''}/bin/flake-fmt";
            meta.description = "Format the repo with treefmt (nix fmt)";
          };
        }
    );

    # Quick formatter for this repo (treefmt + bundled formatters)
    formatter = genAttrs supportedSystems (
      system: let
        pkgs = pkgsFor.${system};
      in
